...
技术标准：
- 多头排列：MA5 > MA10 > MA20
- 乖离率：(Close - MA5) / MA5 < 8%（不追高，适配高波动科技股）
- 量能形态：缩量回调优先
"""
...
class StockTrendAnalyzer:
    ...
    2. 乖离率检测 - 不追高，偏离 MA5 超过 8% 不买（高波动股适配）
    3. 量能分析 - 偏好缩量回调
    ...
    # 交易参数配置
    BIAS_THRESHOLD = 8.0        # 乖离率阈值（%），适配高波动科技股（Mag 7）
    ...
    def _generate_signal(self, result: TrendAnalysisResult) -> None:
        ...
        if bias < 0:
            ...
            score += 4
            risks.append(f"❌ 乖离率过高({bias:.1f}%>8%)，严禁追高！")
