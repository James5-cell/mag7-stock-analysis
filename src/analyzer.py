...
class AnalysisResult:
    ...
    def get_macro_signal(self) -> Dict[str, str]:
        """获取宏观信号数据（用于 Mag 7 宏观分析）"""
        if self.dashboard and 'macro_signal' in self.dashboard:
            return self.dashboard['macro_signal']
        return {}
...
"""
        ...
        if 'trend_analysis' in context:
            ...
            bias_warning = "🚨 超过8%，谨慎追高！" if trend.get('bias_ma5', 0) > 8 else "✅ 安全范围"
            prompt += f"""
### 趋势分析预判（基于交易理念）
| 指标 | 数值 | 判定 |
|------|------|------|
